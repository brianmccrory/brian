...
Most of my articles on Jazz of Japan were about individual albums. During the periods that I was publishing on Substack (in [[/v2-0-substack][v2.0]] and [[/v4-0-substack][v4.0]]), the articles appeared in my Substack archive as a flat list ordered by date with a thumbnail image and an excerpt. To supplement this archive-style index, I wanted to present the articles as a list of albums ordered by album release year, not just by the article post date ([[https://about.jazzofjapan.com/indexes/albums-index/][Albums Index]]). In addition, I wanted to create an index of all musicians included on all albums, with links to each album (each article) that each musician appears on ([[https://about.jazzofjapan.com/indexes/musicians-index/][Musicians Index]]). Finally, I also wanted a timeline-based list of articles to show the oldest to newest issues, like the Substack archive but simpler and customized for my article types ([[https://about.jazzofjapan.com/indexes/timeline/][Publish History Timeline]]).
...
I eventually moved the index pages from Substack to a static site ([[/v3-0-github][v3.0]]), and then moving back to publishing on Substack ([[/v4-0-substack][v4.0]]). This allowed me to automate the process of regenerating index pages and republishing those updates, so that I no longer had to manually update the index pages though cut-and-paste to index pages on Substack.

So, instead of using a Java program to generate index files, I used Jekyll customized templates and layout code (I later changed this custom code [[/from-jekyll-to-hugo][from Jekyll to Hugo]] and Python).

At the same time, I improved the data collection for generating the indexes by parsing my source Markdown files (later, [[/from-markdown-to-org-mode][Org mode files]]) instead of scraped or exported HTML files.
